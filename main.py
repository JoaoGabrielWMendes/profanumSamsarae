...
while True:
    ...
    if not pausado:
            ...
            textoPause = fonteJogo.render("SPACE for pause", True, branco)
            modoTexto = fonteJogo.render(f"Modo: {modoDemiurge}", True, branco)
            screen.blit(karma, (5, 5))
            screen.blit(pecadoAtual, (pecadoX, pecadoY))
            screen.blit(espirito, (espiritoX, 275))
            screen.blit(textoPause, (885, 675))
    fps.tick(60)
    if pausado:
            pausa = fonteJogoPausado.render("PAUSADO", True, vermelho)
            screen.blit(backgroundFrames[backgroundIndex], (movimentoBackground, 0))
            screen.blit(pausa, (280, 300))
